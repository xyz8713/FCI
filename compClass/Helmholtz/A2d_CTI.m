function  A = A2d_CTI( freq, dim, tos, FS, Nz, Nx, N_delta, sigma0, hz, hx, label, Vp, rho, epsilon, delta, theta )
% 2D coupled TI
...
for xx = 1:Nx
    for zz = 1:Nz
        ...
        val_tmp(1,1,1) = val_tmp(1,1,1) + tmp * (rho_tmp(2,1)+rho_tmp(1,2))/(2*rho_tmp(2,1)*rho_tmp(1,2));
        val_tmp(3,1,1) = val_tmp(3,1,1) - tmp * (rho_tmp(2,1)+rho_tmp(3,2))/(2*rho_tmp(2,1)*rho_tmp(3,2));
        val_tmp(1,3,1) = val_tmp(1,3,1) - tmp * (rho_tmp(1,2)+rho_tmp(2,3))/(2*rho_tmp(1,2)*rho_tmp(2,3));
        val_tmp(3,3,1) = val_tmp(3,3,1) + tmp * (rho_tmp(3,2)+rho_tmp(2,3))/(2*rho_tmp(3,2)*rho_tmp(2,3));

        % diagonal term
        val_tmp(2,2,1) = val_tmp(2,2,1) - (hz*freq/vv)^2;

        %% sigma_V: vertial stress
        tmp = -sqrt(1 + 2*dd);
        Czz = tmp * cos(tt)^2;
        Cxx = tmp * sin(tt)^2 * rx^2;
        Czx = tmp * sin(2*tt) * rx;
        ...
        % 3. d^2/dzdx
        tmp = Czx * rho_tmp(2,2) / (4*Sz(zz+tos)*Sx(xx+tos));
        val_tmp(1,1,2) = val_tmp(1,1,2) + tmp * (rho_tmp(2,1)+rho_tmp(1,2))/(2*rho_tmp(2,1)*rho_tmp(1,2));
        val_tmp(3,1,2) = val_tmp(3,1,2) - tmp * (rho_tmp(2,1)+rho_tmp(3,2))/(2*rho_tmp(2,1)*rho_tmp(3,2));
        val_tmp(1,3,2) = val_tmp(1,3,2) - tmp * (rho_tmp(1,2)+rho_tmp(2,3))/(2*rho_tmp(1,2)*rho_tmp(2,3));
        val_tmp(3,3,2) = val_tmp(3,3,2) + tmp * (rho_tmp(3,2)+rho_tmp(2,3))/(2*rho_tmp(3,2)*rho_tmp(2,3));
        ...
        % diagonal term
        val_tmp(2,2,2) = val_tmp(2,2,2) - (hz*freq/vv)^2;
