function  A = A3d_ELP( freq, tos, FS, Nx, Ny, Nz, N_delta, sigma0, hx, hy, hz, label, vel, rho, epsilon, theta, phi )
...
for zz = 1:Nz
    for yy = 1:Ny
        for xx = 1:Nx
            ...
            Cxx = (-1 - 2*ee * ( cos(tt)^2*cos(pp)^2 + sin(pp)^2 )) * rx^2;
            Cyy = (-1 - 2*ee * ( cos(tt)^2*sin(pp)^2 + cos(pp)^2 )) * ry^2;
            Czz =  -1 - 2*ee * sin(tt)^2;
            Cxy = 2*ee * sin(tt)^2 * sin(2*pp) * rx*ry;
            Cyz = 2*ee * sin(2*tt) * sin(pp) * ry;
            Czx = 2*ee * sin(2*tt) * cos(pp) * rx;
