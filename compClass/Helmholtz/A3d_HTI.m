function  A = A3d_HTI( freq, tos, FS, Nx, Ny, Nz, N_delta, sigma0, hx, hy, hz, label, vel, rho, epsilon, delta )
...
for zz = 1:Nz
    for yy = 1:Ny
        for xx = 1:Nx
            ...
            % 7. d^4/dz^2dx^2
            tmp = -2*(ee-dd) * rx^2 / (hz*freq/vel(xx,yy,zz) * Sz(zz+tos) * Sx(xx+tos))^2;
            val_tmp(4)  = val_tmp(4)  + tmp;
            val_tmp(5)  = val_tmp(5)  - tmp*2;
            val_tmp(6)  = val_tmp(6)  + tmp;
            val_tmp(13) = val_tmp(13) - tmp*2;
            val_tmp(14) = val_tmp(14) + tmp*4;
            val_tmp(15) = val_tmp(15) - tmp*2;
            val_tmp(22) = val_tmp(22) + tmp;
            val_tmp(23) = val_tmp(23) - tmp*2;
            val_tmp(24) = val_tmp(24) + tmp;

            % 8. d^4/dz^2dy^2
            tmp = -2*(ee-dd) * ry^2 / (hz*freq/vel(xx,yy,zz) * Sz(zz+tos) * Sy(yy+tos))^2;
            val_tmp(2)  = val_tmp(2)  + tmp;
            val_tmp(5)  = val_tmp(5)  - tmp*2;
            val_tmp(8)  = val_tmp(8)  + tmp;
            val_tmp(11) = val_tmp(11) - tmp*2;
            val_tmp(14) = val_tmp(14) + tmp*4;
            val_tmp(17) = val_tmp(17) - tmp*2;
            val_tmp(20) = val_tmp(20) + tmp;
            val_tmp(23) = val_tmp(23) - tmp*2;
            val_tmp(26) = val_tmp(26) + tmp;
