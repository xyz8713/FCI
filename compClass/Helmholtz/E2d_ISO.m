function  A = E2d_ISO( freq, dim, tos, FS, Nz, Nx, N_delta, sigma0, hz, hx, label, Vp, Vs, rho )
        ...
        for jj = -1:1
            for ii = -1:1
                if zz+ii >= 1 && zz+ii <= Nz && xx+jj >= 1 && xx+jj <= Nx
                    ...
                    C11(ii+2,jj+2) = rho(zz+ii,xx+jj) * Vp(zz+ii,xx+jj)^2;
                    C66(ii+2,jj+2) = rho(zz+ii,xx+jj) * Vs(zz+ii,xx+jj)^2;
                    C33(ii+2,jj+2) = C11(ii+2,jj+2);
                    C13(ii+2,jj+2) = C11(ii+2,jj+2) - 2 * C66(ii+2,jj+2);
                end
            end
        end
