...
while (its < maxits)
    ...
    if ((ro  <=  tol1) | (its >= maxits))
        ...
        if(0)
            %%% harmonic ritz vector version
            HH = hh(1:i+1,1:i)'*hh(1:i+1,1:i);
            GG = hh(1:i+1,1:i)'*vv(:,1:i+1)'*W(:,1:i);
            [Vec,Val] = eig(HH,GG);
            [~,I] = sort(real(diag(Val)));
            Ndef = min(ITopts.Nevec,i);
            U = Vec(:,I(1:Ndef));
            U = qr(U,0);
            U = W(:,1:i)*U;
        else
            ...
            GG = vv(:,1:i)'*W(:,1:i);
            [Vec,Val] = eig(HH,GG);
            %%            [~,I] = sort(real(diag(Val)), 'descend'
            %%            );
            [~,I] = sort(abs(diag(Val)), 'descend' );
