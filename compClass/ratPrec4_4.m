...
%%--------------------
 ...
 %% method = 2 ; shift is reduced at each next circle 
 %% method = 3 ; uses CGNR for B-shift I instead of fgmres
%%--------------------
opts   = prec.ITopts;
method = opts.outer;
Nlev  = prec.Nlev;
...
%%-------------------- Solve Ax = P y2
    ...
    elseif(method == 3) 
        ...
        it = opts.maxits*lev;
        tol = opts.tolIts;
        [y3,res2] = CGNR (B,zeros(n,1), y2, it, tol) ;
    end
    y =  y + (y1 + y3);
    %%# debug 
    %% - ex = B\ones(n,1);
